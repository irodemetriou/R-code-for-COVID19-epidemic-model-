...
######################################
# Auxiliary Particle Filtering using bootstrap proposal 
wanorm <- matrix(0, T+1, N)
XAP <- matrix(0, T+1, N)
xabar <- matrix(0, T+1, N)
logmarginal_ap <- vector("numeric")
ESSAP <-vector("numeric")
ap_mean <- vector("numeric")
ap_var <- vector("numeric")

for (i in 1:(T)){
  if (i == 1){
    XAP[i,] <- rnorm(N)
    wa <- dnorm(Y[i], mean = XAP[i,], sd = sigma)*dnorm(Y[i+1], mean = rho*XAP[i,], sd = sqrt(tau^2 + sigma^2))
    wanorm[i,] = wa/sum(wa) # check weights? 
    # resample 
    rs <- sample(XAP[i,], size = N, replace = TRUE, prob = wa)
    XAP[i,] <- rs
    xabar[i,] <- XAP[i,]
  
  } else{
    XAP[i,] <- rnorm(N, mean = rho*XAP[i-1,], sd= tau)
    wa <- dnorm(Y[i], mean = XAP[i,], sd = sigma)*dnorm(Y[i+1], mean = rho*XAP[i,], sd = sqrt(tau^2 + sigma^2))/dnorm(Y[i], mean = rho*XAP[i-1,], sd = sqrt(tau^2 + sigma^2))
    wanorm[i,] = wa/sum(wa)
    
    for (j in 1:i){
      #  resample 
      rs <- sample(XAP[j,], size = N, replace = TRUE, prob = wa)
      XAP[j,] <- rs
    }
    xabar[i,] <- XAP[i,]
  }
  ESSAP[i] = 1/sum(wanorm[i,]^2)
  ap_mean[i] <-sum(wanorm[i,]*XAP[i,]) 
  ap_var[i] <- sum(wanorm[i,]*XAP[i,]^2) - ap_mean[i]^2 
  
}


apdf<- data.frame(x =1:T, AF_Mean = ap_mean)
ggplot(apdf, aes(x)) + geom_line(aes(y = AF_Mean), colour = "purple")


plot(XAP[,1], type = "l", xlab = "Time", ylab = "X", ylim = c(-11,11), 
     col = "lightgrey", main = "Bootstrap Proposal for AP with KF mean")
for (i in 2:ncol(XAP)){
  lines(XAP[,i], lwd = 0.3, col = "lightgrey")
}
lines(kf_mean, lwd = 1.5, col="orange")
plot(ap_var)

datf <- data.frame(x, ESS = ESSAP)
ggplot(datf, aes(x)) + geom_line(aes(y = ESSAP), colour = "orange")+
  ggtitle("Estimate of ESS for Auxiliary Particle Filter") +
  theme(plot.title = element_text(hjust = 0.5)) + 
  xlab("time") + ylab("ESS") 

dm <- data.frame(x, apmean = ap_mean, kfmean = kf_mean[1:T])
ggplot(dm, aes(x)) + geom_line(aes(y = apmean), colour = "red")+
  geom_line(aes(y = kfmean), colour = "black")+
  ggtitle("Estimate of filter mean for Auxiliary Particle Filter") +
  theme(plot.title = element_text(hjust = 0.5)) + 
  xlab("time") + ylab("ESS") 

x <- 1:(T)
df_means2 <- data.frame(x, kf_mean= kf_mean[1:T], sis_mean = sis_mean[1:T], sir_mean = sir_mean[1:T], ap_mean = ap_mean, Y = Y[1:T])
ggplot(df_means2, aes(x)) +
  geom_jitter(aes(y=kf_mean), colour="red") + 
  geom_line(aes(y=sis_mean), colour="purple") +
  geom_line(aes(y=sir_mean), colour="black") + 
  geom_line(aes(y=ap_mean), colour="green") +
  geom_line(aes(y=Y), colour="blue") +
  ggtitle("Estimates of filter mean for bootstrap proposal") +
  theme(plot.title = element_text(hjust = 0.5)) + 
  xlab("time") + ylab("mean") 

df_means2b <- data.frame(x, kf_mean= kf_mean[1:T], sis_mean = sis_mean2[1:T], sir_mean = sir_mean2[1:T], ap_mean = ap_mean, Y = Y[1:T])
ggplot(df_means2b, aes(x)) +
  geom_jitter(aes(y=kf_mean), colour="red") + 
  geom_line(aes(y=sis_mean), colour="purple") +
  geom_line(aes(y=sir_mean), colour="black") + 
  geom_line(aes(y=ap_mean), colour="green") +
  geom_line(aes(y=Y), colour="blue") +
  ggtitle("Estimates of filter mean for optimal proposal") +
  theme(plot.title = element_text(hjust = 0.5)) + 
  xlab("time") + ylab("mean") 
